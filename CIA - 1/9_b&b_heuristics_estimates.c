...
void displayGraph() {
    ...
    printf("\nHeuristics:\n");
    for (int i = 0; i < numVertices; i++) {
        printf("Heuristic for %c: %d\n", vertices[i], heuristic[i]);
    }
}

// Recursive Branch and Bound function with heuristics
void branchAndBoundHeuristic(int current, int goal, int cost, int depth) {
    if (cost + heuristic[current] >= bestCost) {
        return; // Prune branch if cost + heuristic exceeds best cost
    }
    ...
    for (int i = 0; i < numVertices; i++) {
        if (adjMatrix[current][i] != 0 && !visited[i]) {
            visited[i] = true;
            currentPath[depth] = i;
            branchAndBoundHeuristic(i, goal, cost + adjMatrix[current][i], depth + 1);
            visited[i] = false;
        }
    }
}
...
int main() {
    ...
    // Perform Branch and Bound Search with heuristics
    branchAndBoundHeuristic(source, destination, 0, 1);
