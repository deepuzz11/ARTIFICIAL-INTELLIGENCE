...
def a_star_search(graph, start, goal, heuristic):
    frontier = [(0 + heuristic[start], 0, start, [start])]
    visited = set()
    
    while frontier:
        est_total_cost, cost_so_far, node, path = heapq.heappop(frontier)
        ...
        for neighbor in graph.neighbors(node):
            if neighbor not in visited:
                ...
                heapq.heappush(frontier, (est_total_cost, total_cost, neighbor, path + [neighbor]))
...
# Perform A* Search
path = a_star_search(G, start_vertex, goal_vertex, heuristic)
