...
void branchAndBound(int current, int goal, int cost, int depth) {
    if (cost >= bestCost) {
    ...
    for (int i = 0; i < numVertices; i++) {
        if (adjMatrix[current][i] != 0 && !visited[i]) {
            visited[i] = true;
            currentPath[depth] = i;
            branchAndBound(i, goal, cost + adjMatrix[current][i], depth + 1);
            visited[i] = false;
        }
    }
}
...
int main() {
    ...
    branchAndBound(source, destination, 0, 1);
